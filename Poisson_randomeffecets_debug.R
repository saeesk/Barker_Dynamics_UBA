...
for( i in 1 : length(timesteps))
{
  ...
  ULA_mu = mean(r1[,1])
  UBA_mu = mean(r2[-foo,1])
  errL = (ULA_mu-mustar)^2
  errB = (UBA_mu-mustar)^2
...
dev.off()
write.csv(err_mat, file = 'errors_warm_v3.csv')
...
for( i in 1 : length(timesteps))
{
  r1= ULA(x0,  dt = timesteps[i], N = 5e5)
  r2 = UBA(x0,  dt = timesteps[i], N = 6e4)
  ULA_mu = mean(r1[,1])
  UBA_mu = mean(r2[-foo,1])
  errL = (ULA_mu-mustar)^2
...
legend('topright' , legend = c('ULA' , 'UBA') , col = c('black' , 'red') ,lty = 1 )
dev.off()
write.csv(err_mat, file = 'errors_fixed_v3.csv')
#
